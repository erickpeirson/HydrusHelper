...
# Convert an array of floats to an array of strings, formatted in scientific notation
...
class WPathMixin(object):
    ...
    def _checkpath(self, fpath):
        ...
        if not os.path.exists(spath):
            raise RuntimeError("Couldn't find {0} in {1}.".format(spath, self.wpath))    
        return spath
...
class Cloneable(object):
    ...
    def write(self):
        if not hasattr(self, 'render'):
            raise AttributeError('Subclass of Cloneable must provide method `render`')
...
class SELECTORHandler(DataReader):
    template = '\n'.join(
        ['Pcp_File_Version=4',
         ...
         ' {lSnow}       {lHP1}       {lMeteo}       {lVapor}       {lActiveU}       {lFluxes}       {lIrrig}       f       f       f',
         'NMat    NLay  CosAlpha',
         '  {NMat}       {NLay}       {CosAlpha}',
...
class ATMOSPHHandler(DataReader, Cloneable):
    """
    ATMOSPH.IN
    """
    delim = '   '
    template = '\n'.join([
        ...
        '       '.join(["","{DailyVar}","{SinusVar}","{lLay}","{lBCCycles}","{lInterc}","{lDummy1}","{lDummy2}","{lDummy3}","{lDummy4}","{lDummy5}"]),
    ...
    def render(self):
        ...
        fdict.update({'headers': self.delim + self.delim.join(self.headers + ['RootDepth']), 'data':self.data})
        try:
            formatted = self.template.format(**fdict)
        except AttributeError:
            raise RuntimeError('Required parameters not set.')
                
        return formatted            

class ProfileHandler(DataReader, Cloneable):
    template = '\n'.join([
        "Pcp_File_Version=4",
        "    2",
        "    1  {x_sfc}  1.000000e+000  1.000000e+000",
        "    2  {x_dpt}  1.000000e+000  1.000000e+000",
        "   {nodes}   {p0}   {p1}   {p2}   {headers}",
        "{data}",
        "   {nodes}",
        "   {node_numbers}"  ])
    delim = '   '
...
class SimulationHandler(WPathMixin):
    ...
    def __init__(self, wpath, **kwargs):
        ...
        if not hasattr(self, 'simpath'):
            raise ValueError('Please provide a path at which to save simulation workspaces.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n', metavar='name', required=True, help='Provide a name for this simulation set')    
    parser.add_argument('-i', metavar='input-path', required=True, help='Path to input directory')
    parser.add_argument('-s', metavar='sim-path', required=True,
        help='Path to directory where simulation workspaces will be created')
    parser.add_argument('-o', metavar='output-path', required=True,
        help='Path to directory where output will be stored')
    parser.add_argument('--chain-theta', action='store_const', const=True, default=False,
        help=''.join(
            ['If provided, final theta values from Obs_Node.out in one simulation will',
             ' be used as values of h in PROFILE.DAT in the next simulation']))
